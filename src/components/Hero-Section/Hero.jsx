...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-[90vh] md:min-h-screen flex items-center bg-background overflow-hidden pt-10 md:pt-0">
      ...
      <div className={`absolute inset-0 bg-gradient-to-br ${careerData[index].color} to-transparent opacity-30 transition-colors duration-1000 -z-10`} />
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
          ...
          <div className="w-full lg:w-1/2 order-2 lg:order-1 text-center lg:text-left space-y-6 md:space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
              <Button size="lg" className="w-full sm:w-auto rounded-full h-14 px-10 text-lg font-bold shadow-xl hover:shadow-primary/20 transition-all active:scale-95">
                রোডম্যাপ দেখুন <ArrowRight className="ml-2 w-5 h-5" />
              </Button>
              <Button size="lg" variant="outline" className="w-full sm:w-auto rounded-full h-14 px-8 text-lg font-bold border-2 border-border hover:bg-secondary">
              ...
              {careerData.map((_, idx) => (
                <button
                  ...
                  className={`h-2.5 transition-all duration-500 rounded-full ${
                    index === idx ? "w-10 bg-primary" : "w-2.5 bg-primary/20 hover:bg-primary/40"
                  }`}
                />
              ))}
            </div>
